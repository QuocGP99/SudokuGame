...
export const createSudokuGame = (level: number) => {
  const fullBoard = generateFullBoard();
  const gameBoard: SudokuBoard = fullBoard.map(row => [...row]);
  
  // Logic: Level 1 (30 hidden), Level 5 (45 hidden), Level 10 (60 hidden)
  // Linear scale: hidden = 30 + (level - 1) * (30/9)
  const cellsToRemove = Math.floor(30 + (level - 1) * 3.33);
